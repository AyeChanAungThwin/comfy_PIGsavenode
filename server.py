...
class PIGServer:
    ...
    async def rename_category(self, request):
        """重命名分类"""
        try:
            body = await request.json()
            ...
            # 检查新名称是否已存在
            if new_name.strip() in data['categories']:
                return web.json_response({
                    'success': False,
                    'error': '分类名称已存在'
                }, status=400)
            ...
            # 重命名分类
            if old_name in data['categories']:
                old_index = data['categories'].index(old_name)
                data['categories'][old_index] = new_name.strip()
            
            # 更新该分类下所有收藏的分类名称
            for fav in data['favorites']:
                if fav.get('category') == old_name:
                    fav['category'] = new_name.strip()
    ...
    async def reorder_favorites(self, request):
        """重新排序收藏"""
        try:
            body = await request.json()
            order = body.get('order', [])  # 收藏ID的排序列表
            ...
            # 创建ID到收藏的映射
            fav_dict = {fav.get('id'): fav for fav in data['favorites']}
            
            # 按新顺序重新排列
            ordered_favs = []
            for fav_id in order:
                if fav_id in fav_dict:
                    ordered_favs.append(fav_dict[fav_id])
            
            # 添加不在排序列表中的收藏（保持原有顺序）
            for fav in data['favorites']:
                if fav.get('id') not in order:
                    ordered_favs.append(fav)
            
            data['favorites'] = ordered_favs
